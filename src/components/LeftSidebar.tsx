...
const NAV_ITEMS = [
  ...
  { icon: Bookmark, label: 'Bookmarks', action: null },
  { icon: PenTool, label: 'Creator Studio', action: null },
...
interface SidebarProps {
  onHomeClick?: () => void;
  analyzedProfile?: { name: string; username: string; avatar: string } | null;
}

export function LeftSidebar({ onHomeClick, analyzedProfile }: SidebarProps) {
  ...
  return (
    <header className='flex flex-col items-end pr-3 sticky top-0 h-screen'>
      <div className='flex flex-col w-[260px] h-full py-1'>
        {/* X Logo + branding */}
        <div className='px-3 py-3'>
          ...
          >
            <span className='text-[19px] font-bold text-x-text hidden xl:inline'>
              idify your
            </span>
            {showBird ? (
              <svg
                viewBox='0 0 24 24'
                className='w-6 h-6 fill-x-blue animate-fade-in'
              >
              ...
              <svg
                viewBox='0 0 24 24'
                className='w-6 h-6 fill-x-text transition-transform'
                style={{
                  transform:
                    logoSpins > 0 ? `rotate(${logoSpins * 360}deg)` : undefined,
                  transition:
        ...
        {/* Nav Items */}
        <nav className='flex flex-col items-start gap-[6px] mt-1'>
          {NAV_ITEMS.map(item => {
            ...
            return (
              <button
                ...
                className='inline-flex items-center gap-4 pl-4 pr-6 py-3 rounded-full hover:bg-[#181919] transition-colors group cursor-pointer relative'
              >
                <div
                  className='relative'
                  style={
                ...
                >
                  <Icon
                    className='w-[26px] h-[26px] text-x-text'
                    strokeWidth={1.75}
                  />
                  {isNotif && notifBadge && (
                ...
                <span className='text-[20px] font-medium text-x-text hidden xl:inline'>
                  {item.label}
                </span>
              </button>
            );
        ...
        {/* Account — shows analyzed profile or cycles through funny identities */}
        <button
          onClick={analyzedProfile ? undefined : handleIdentityClick}
          className={`mt-auto mb-3 mx-3 px-3 py-3 rounded-full hover:bg-[#181919] transition-all cursor-pointer flex items-center gap-3 ${
            !analyzedProfile && identityShake
              ? 'animate-[wiggle_0.4s_ease]'
              : ''
          }`}
        >
          {analyzedProfile?.avatar ? (
            <Image
              src={analyzedProfile.avatar}
              alt={analyzedProfile.name}
              width={40}
              height={40}
          ...
          <div className='hidden xl:block flex-1 min-w-0 text-left'>
            <p className='text-[15px] font-bold text-x-text truncate'>
              {analyzedProfile ? analyzedProfile.name : identity.name}
            </p>
            <p className='text-[15px] text-x-secondary truncate'>
              {analyzedProfile
                ? `@${analyzedProfile.username}`
                : identity.handle}
            </p>
          </div>

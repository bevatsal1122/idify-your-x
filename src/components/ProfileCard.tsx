...
export function ProfileCard({ profile, summary, analyzedTweetsCount, analyzedHighlightsCount, onBack }: Props) {
  return (
    <div className="animate-slide-up">
      {/* Profile header bar — like X's profile page */}
      <div className="sticky top-0 z-20 bg-x-bg/80 backdrop-blur-md px-4 h-[53px] flex items-center gap-7">
        ...
        <div>
          <div className="flex items-center gap-1">
            <h2 className="text-[17px] font-extrabold text-x-text leading-5">{profile.name}</h2>
      ...
      <div className="px-4">
        <div className="flex justify-between items-start">
          ...
          <div className="-mt-[67px] relative">
            {profile.avatar ? (
              <Image
                src={profile.avatar}
                alt={profile.name}
                width={134}
                height={134}
                className="rounded-full border-[4px] border-x-bg"
              />
            ) : (
              <div className="w-[134px] h-[134px] rounded-full border-[4px] border-x-bg bg-x-tertiary" />
            )}
          </div>
          {/* Button aligned to bottom of avatar area */}
          <div className="mt-3">
            <button className="px-4 py-1.5 border border-x-border rounded-full text-[14px] font-bold text-x-text hover:bg-[#181919] transition-colors cursor-pointer">
        ...
        {/* Meta row */}
        <div className="flex items-center gap-3 text-[15px] text-x-secondary flex-wrap mb-3">
          {profile.location && (
      ...
      {/* Tabs — X profile style */}
      <div className="flex border-b border-x-border">
        {['Posts', 'Replies', 'Highlights', 'Articles', 'Media', 'Likes'].map((tab) => (
          <button
            key={tab}
            className={`flex-1 py-4 text-center text-[15px] hover:bg-[#181919] transition-colors relative cursor-pointer ${
